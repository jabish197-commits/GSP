...
function Header({ cartCount, customer, logout, settings }) {
  ...
  return <header className="site-header"><Link className="brand" to="/"><img src="/logo.png" alt="SJ Guppy Paradise"/><span><b>SJ GUPPY</b><small>PARADISE</small></span></Link><button className="menu" onClick={() => setOpen(!open)} aria-label="Toggle menu">☰</button><nav className={open ? "open" : ""}><Link to="/">Home</Link><Link to="/collection">Guppies</Link><a href="/#care">Care guide</a><a href="/#contact">Contact</a>{settings.instagramUrl&&<a className="instagram-nav" href={settings.instagramUrl} target="_blank" rel="noreferrer">Instagram</a>}<Link className="nav-cart" to="/enquiry">Enquiry <span>{cartCount}</span></Link>{customer?<><Link className="account-link" to="/account">Hi, {customer.name.split(" ")[0]}</Link><button className="nav-logout" onClick={logout}>Logout</button></>:<><Link to="/login">Login</Link><Link className="nav-register" to="/register">Register</Link></>}</nav></header>;
}
...
function FishDetails({ fish, add }) {
  ...
  return <main className="detail"><FishGallery media={item.media} name={item.name}/><div className="detail-copy"><button className="back" onClick={() => navigate(-1)}>← Back</button><p className="eyebrow">{item.strain}</p><h1>{item.name}</h1><strong className="detail-price">{money(item.price)}</strong><p>{item.description}</p><dl><div><dt>Selection</dt><dd>{item.sex}</dd></div><div><dt>Availability</dt><dd>{item.quantity} · {item.status}</dd></div><div><dt>Breeder support</dt><dd>Included</dd></div></dl><button className="primary full" onClick={() => add(item)}>Add to enquiry</button><small>{"Final availability and delivery are confirmed personally before payment."}</small></div></main>;
}
...
function PaymentConfirmation({ order, customer, settings }) {
  const [busy,setBusy]=useState(false),[error,setError]=useState(""),[sent,setSent]=useState(false),[preview,setPreview]=useState("");
...
function CustomerAccount({ customer, logout }) {
  const [orders, setOrders] = useState([]), [loading, setLoading] = useState(true), [error, setError] = useState("");
  useEffect(() => { api("/orders/mine").then(data=>setOrders(data.orders)).catch(value=>setError(value.message)).finally(()=>setLoading(false)); }, []);
  return <main className="page account-page"><section className="account-profile"><p className="eyebrow">MY ACCOUNT</p><h1>{customer.name}</h1><p>{customer.email}</p><p>{customer.phone}</p><button onClick={logout}>Logout</button></section><section className="account-orders"><h2>My enquiries</h2>{loading&&<p>Loading enquiries…</p>}{error&&<p className="error">{error}</p>}{!loading&&!orders.length&&<div className="account-empty"><p>You have not sent an enquiry yet.</p><Link className="primary" to="/collection">Explore guppies</Link></div>}{orders.map(order=><article key={order._id}><div><b>{order.orderNumber}</b><small>{new Date(order.createdAt).toLocaleDateString()}</small></div><div><span>{order.items.map(item=>`${item.name} · ${item.selectionType||"pair"} · ${item.totalFish||item.quantity} fish`).join(", ")}</span><strong>{money(order.total)}</strong></div><div><i className={`account-status ${order.status}`}>{order.status}</i><small>Payment: {order.paymentStatus||"pending"}</small></div></article>)}</section></main>;
}
...
function ChatWidget({ customer }) {
  const [open, setOpen] = useState(false), [chat, setChat] = useState(null), [text, setText] = useState("");
