...
function CustomerAccount(props) {
  ...
  return <><section className="page payment-tracking-page"><div className="payment-tracking-title"><div><p className="eyebrow">LIVE PAYMENT TRACKING</p><h1>Track my payments</h1><p>Status refreshes automatically every 10 seconds.</p></div><button type="button" onClick={load} disabled={refreshing}>{refreshing?"Refreshing…":"↻ Refresh status"}</button></div>{loading?<p>Loading payment tracking…</p>:!orders.length?<p className="account-empty">No payments to track yet.</p>:<div className="payment-tracking-grid">{orders.map(order=><article key={order._id}><div className="tracking-order-head"><div><b>{order.orderNumber}</b><small>{new Date(order.createdAt).toLocaleDateString()}</small></div><strong>{money(order.total)}</strong></div><PaymentTracker order={order}/></article>)}</div>}</section><CustomerAccountContent {...props}/></>;
}
