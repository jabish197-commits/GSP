...
function OrderTable({orders,update,updatePayment}) {
  return <div className="table-wrap"><table><thead><tr><th>Reference</th><th>Customer</th><th>Items</th><th>Value</th><th>Order</th><th>Payment</th>{update&&<th>Actions</th>}</tr></thead><tbody>{orders.map(order=><tr key={order._id}>
    <td><b>{order.orderNumber}</b><small>{new Date(order.createdAt).toLocaleDateString()}</small></td>
    <td>{order.customer.name}<small>{order.customer.phone}</small></td>
    <td>{order.items.map(i=>`${i.name} · ${i.selectionType||"pair"} · ${i.totalFish||i.quantity} fish`).join(", ")}</td>
    <td>{money(order.total)}</td>
    <td>{update?<select value={order.status} onChange={e=>update(order._id,e.target.value)}>{["new","confirmed","preparing","shipped","completed","cancelled"].map(s=><option key={s}>{s}</option>)}</select>:<i className={order.status}>{order.status}</i>}</td>
    <td><i className={order.paymentStatus||"pending"}>{order.paymentStatus||"pending"}</i>{order.paymentProof?.url&&<small><a href={order.paymentProof.url} target="_blank" rel="noreferrer" style={{color:"var(--blue)",fontWeight:700}}>View screenshot</a></small>}{updatePayment&&order.paymentStatus==="submitted"&&<div style={{display:"flex",gap:4,marginTop:6}}><button onClick={()=>updatePayment(order._id,"verified")}>Verify</button><button className="danger" onClick={()=>updatePayment(order._id,"rejected")}>Reject</button></div>}</td>
    {update&&<td>{order.status==="new"&&!order.acceptedAt?<button onClick={()=>update(order._id,"confirmed")} style={{border:0,borderRadius:7,padding:".65rem .9rem",background:"var(--orange)",color:"white",fontWeight:800,cursor:"pointer",whiteSpace:"nowrap"}}>Accept request</button>:<span className="order-accepted">✓ Accepted</span>}</td>}
  </tr>)}{!orders.length&&<tr><td colSpan={update?"7":"6"}>No enquiries yet.</td></tr>}</tbody></table></div>;
}
function Orders({orders,reload}) {
  const [acceptedIds,setAcceptedIds]=useState([]);
  const visibleOrders=orders.map(order=>acceptedIds.includes(order._id)?{...order,status:"confirmed",acceptedAt:new Date().toISOString()}:order);
  const update=async(id,status)=>{
    if(status==="confirmed") setAcceptedIds(ids=>ids.includes(id)?ids:[...ids,id]);
    try { await api(`/orders/${id}/status`,{method:"PATCH",body:JSON.stringify({status})}); await reload(); }
    catch(error){ if(status==="confirmed")setAcceptedIds(ids=>ids.filter(value=>value!==id)); throw error; }
  };
  const updatePayment=async(id,paymentStatus)=>{const label=paymentStatus==="verified"?"Optional verification note":"Reason for rejection (shown to customer)";const adminNote=window.prompt(label,"");if(adminNote===null)return;await api(`/orders/${id}/payment-status`,{method:"PATCH",body:JSON.stringify({paymentStatus,adminNote})});reload()};
  return <section className="panel"><div className="panel-head"><div><p className="kicker">CUSTOMER REQUESTS</p><h2>All enquiries</h2></div></div><OrderTable orders={visibleOrders} update={update} updatePayment={updatePayment}/></section>;
}
