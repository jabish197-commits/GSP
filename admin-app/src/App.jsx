...
function Chats({chats,reload}) {
  ...
  return <section className="chat-desk">
    ...
    <main>{current?<>
      <header><div><h2>{current.customerName}</h2><p>{current.customerPhone||"No phone shared"} · {current.status}</p></div><div>{current.status==="pending"&&<button className="main-action" style={{background:"var(--orange)",color:"white",borderColor:"var(--orange)"}} onClick={()=>status("accepted")}>Accept request</button>}{current.status!=="closed"&&<button onClick={()=>status("closed")}>Close</button>}</div></header>
