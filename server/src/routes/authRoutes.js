...
router.post("/login", asyncHandler(async (request, response) => {
  ...
  response.cookie("sj_admin_token", token, sessionCookieOptions(8 * 60 * 60 * 1000));
...
router.post("/logout", (_request, response) => response.clearCookie("sj_admin_token", clearSessionCookieOptions()).json({ message: "Logged out." }));
